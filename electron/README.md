...
- 🔗 **قاعدة بيانات موحّدة مع Android** — كل طلبات `/api/*` تُوجَّه تلقائياً إلى خادم الإنتاج، بحيث تعمل نسخة ويندوز ونسخة Android على **نفس قاعدة البيانات تماماً**.
...
## 🔀 كيف يعمل توحيد قاعدة البيانات مع Android؟

منصة Emergent تفصل بين نطاقين:
- **رابط الواجهة (preview)** — يقدّم HTML و JavaScript للتطبيق. تُخدَم منه ملفات الواجهة فقط.
- **خادم الإنتاج (production)** — يستضيف FastAPI و MongoDB الفعلية. Android APK يتصل به مباشرة.

Electron يحمّل الواجهة من رابط preview (لأن الإنتاج لا يقدّم HTML)، ثم يعترض بشكل شفّاف كل طلب `/api/*` ويعيد توجيهه إلى خادم الإنتاج. النتيجة: نفس قاعدة البيانات على Android و Windows، بدون أي إعداد إضافي من المستخدم.

الرابطان مضبوطان تلقائياً كإعدادات افتراضية في `main.js`:
```js
DEFAULT_PRODUCTION_API_URL = 'https://pharma-checkout-8.emergent.host';
DEFAULT_FRONTEND_URL       = 'https://pharma-checkout-8.preview.emergentagent.com';
```

للتعديل: افتح **الإعدادات (Ctrl+,)** داخل التطبيق.
